...
public class MainView extends View {
	...
	public void drawScoreText(Canvas canvas) {
		...
		canvas.drawText("HIGH SCORE", sXHighScore + textMiddleHighScore,
				titleStartYAll, paint);
		paint.setTextSize(bodyTextSize);
		paint.setColor(TEXT_WHITE);
		canvas.drawText("" + game.highScore, sXHighScore + textMiddleHighScore,
				bodyStartYAll, paint);
		...
		paint.setTextSize(titleTextSize);
		paint.setColor(TEXT_BROWN);
		canvas.drawText("SCORE", sXScore + textMiddleScore, titleStartYAll,
				paint);
		paint.setTextSize(bodyTextSize);
		paint.setColor(TEXT_WHITE);
		canvas.drawText("" + game.score, sXScore + textMiddleScore,
				bodyStartYAll, paint);
	}

	public void drawAutoRunButton(Canvas canvas) {
		drawDrawable(canvas, backgroundRectangle, sXUndo - 300, sYIcons, sXUndo
				+ iconSize - 300, sYIcons + iconSize);
	...
	public void drawInstructions(Canvas canvas) {
		...
		paint.setTextAlign(Paint.Align.LEFT);
		int textShiftY = centerText() * 2;
		canvas.drawText("Swipe to move. 2+2 = 4. Reach 2048.", startingX,
	...
	public void drawEndGameState(Canvas canvas) {
		...
		if (game.won) {
			...
			drawDrawable(canvas, lightUpRectangle, startingX, startingY,
					endingX, endingY);
			...
			paint.setTextAlign(Paint.Align.CENTER);
			canvas.drawText("You Win!", boardMiddleX, boardMiddleY
					- centerText(), paint);
			paint.setAlpha(255);
		} else if (game.lose) {
			fadeRectangle.setAlpha((int) (127 * alphaChange));
			drawDrawable(canvas, fadeRectangle, startingX, startingY, endingX,
					endingY);
			...
			paint.setTextSize(gameOverTextSize);
			paint.setTextAlign(Paint.Align.CENTER);
			canvas.drawText("Game Over!", boardMiddleX, boardMiddleY
					- centerText(), paint);
			paint.setAlpha(255);
		}
	}
	...
	public void getLayout(int width, int height) {
		...
		startingX = boardMiddleX - (cellSize + gridWidth) * halfNumSquaresX
				- gridWidth / 2;
		endingX = boardMiddleX + (cellSize + gridWidth) * halfNumSquaresX
				+ gridWidth / 2;
		startingY = boardMiddleY - (cellSize + gridWidth) * halfNumSquaresY
				- gridWidth / 2;
		endingY = boardMiddleY + (cellSize + gridWidth) * halfNumSquaresY
				+ gridWidth / 2;
